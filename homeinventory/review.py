...
class ReviewHandler(BaseHandler):
    ...
    def _render_start(self, st: SessionState) -> str:
        proj = self.project
        env = Environment(loader=FileSystemLoader(TEMPLATES),
                          autoescape=select_autoescape(["html"]))
        picker = (not proj.project_path.exists() and not proj.is_legacy
    ...
    def _route_get(self):
        ...
        # tenant routes (single-session legacy layout)
        if not proj.is_multi:
            ...
            if m:
                if not self._tenant_token_ok(m.group(1)):
                    self._err(403, "invalid or expired link")
                    return
                self._html(self._render_app(st, "share.html.j2", token=m.group(1)))
                return
    ...
    def _mutate(self):
        ...
        try:
            ...
            if path == "/api/pdf":
                ...
                pdf_url = f"{st.route_prefix}/pdf" if st.route_prefix else "/pdf"
                self._json({"ok": True, "pdf": pdf_url})
                return
            if path == "/api/inventory":
                body = self._body()
